...
def run_true_batch(tasks):
    """Run real batched LLaVA generation.

    Requires models.get_llava_answer_batch from the patched models.py. It returns
    a list with one answer-list per request, for example [[answer1], [answer2]].
    """
    args = tasks[0].args
    images = [task.image for task in tasks]
    prompts = [task.args.prompt for task in tasks]

    if 'get_llava_answer_batch' not in globals():
        raise RuntimeError("models.get_llava_answer_batch is not available; use the patched models.py")

    with torch.inference_mode():
        batched_answers = get_llava_answer_batch(
            mistral_model,
            mistral_tokenizer,
    ...
    if not isinstance(batched_answers, (list, tuple)) or len(batched_answers) != len(tasks):
        raise RuntimeError(
            f"True batch returned {type(batched_answers).__name__} with length "
            f"{len(batched_answers) if hasattr(batched_answers, '__len__') else 'unknown'} for {len(tasks)} tasks"
        )

    for task, answer_list in zip(tasks, batched_answers):
        if isinstance(answer_list, (list, tuple)):
            task.answer = answer_list[0] if answer_list else None
        else:
